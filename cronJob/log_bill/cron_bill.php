...
while ($row = mysqli_fetch_array($result)) {
    ...
    $sql_qrcode = "SELECT * FROM districts WHERE id = $dis_id";
    $result_sql_qrcode = $con->query($sql_qrcode);
    $qr = mysqli_fetch_array($result_sql_qrcode);
    // while ($qr = mysqli_fetch_array($result_sql_qrcode)) {
    ...
    $sql_bill = "INSERT INTO payment_logs (tran_key, month, year, student_id, pm_status_id, qr_code, created_at)
VALUES ('$tran_key', '$month', '$year', '$student_id', '1', '$image_qr', '$full')";
    $con->query($sql_bill);
}
